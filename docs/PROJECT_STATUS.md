...
### What's NOT Tested
- ⚠️ Zero-diff under sustained *live* redstone activity — **first live-load (driven) evidence captured 2026-07-09, see below**. The 10k-tick DG1 Criterion 1 run still captures a **static** world (see the DG1 Criterion 1 note); a driven `--drive <seed>` mode now exists and has produced byte-identical same-seed captures on a real toggled lever, but only at small scale (1 source / 200 ticks) so far — the 10k-tick multi-region driven run is the remaining work.

  **DRIVEN LIVE-LOAD ZERO-DIFF — first evidence (2026-07-09).** The pure driver stack (`DeterministicToggleSchedule` → `LiveLoadToggleDriver` → `CanonicalToggleSources` → `FoliaToggleApplier`) is now wired end-to-end through the `FoliaCaptureHarness.TickDriver` seam via `/nebula capture start <ticks> --drive <seed> [--period <n>]`. On real Folia 26.1.2, a lever→6-wire→lamp circuit was scanned (`/nebula status` → toggle sources = 1) and driven three times with the same seed (42, period 8, 200 ticks). Result: **runs 2 and 3 were byte-for-byte identical across all 200 frames** (sha256 `1cfe3caf…`). Run 1 (the COLD run, empty CAS store) differed only in its **first 13 frames**, then converged to the identical steady-state trajectory and stayed locked for frames 13–199. This is the expected warm-up transient: the driver's toggle stream is deterministic (proven by the byte-identical warm runs), and the only run-to-run difference is the initial CAS state, which settles within ~13 ticks. `/nebula perf` recorded 306 DAG ticks over the driven runs, max microsteps 2. This is the FIRST zero-diff evidence under *driven live redstone*, distinct from the static-world Criterion 1 result.
