...
### What's NOT Tested
- ⚠️ Zero-diff under sustained *live* redstone activity — **driven evidence now AUTOMATED in the harness, 2026-07-09, see below**. The 10k-tick DG1 Criterion 1 run still captures a **static** world (see the DG1 Criterion 1 note); a driven `--drive <seed>` mode now exists in both the plugin AND `scripts/zerodiff-harness.sh`, and has produced a reproducible convergent-identical result on real toggled levers — but only at small scale (2 sources / 200 ticks) so far, so the 10k-tick multi-region driven run is the remaining work.

  **DRIVEN LIVE-LOAD ZERO-DIFF — now automated end-to-end (2026-07-09).** The pure driver stack (`DeterministicToggleSchedule` → `LiveLoadToggleDriver` → `CanonicalToggleSources` → `FoliaToggleApplier`) is wired through the `FoliaCaptureHarness.TickDriver` seam via `/nebula capture start <ticks> --drive <seed> [--period <n>]`, and `scripts/zerodiff-harness.sh --drive <seed>` now drives the whole experiment: it heads each region-spaced circuit with a real LEVER, `/nebula scan`-registers them as toggle sources, runs a throwaway warm-up capture to prime the CAS store, resets+settles the levers, then runs two seeded driven captures and **grades them at the FRAME level** (not just a bare sha256). Verified on real Folia 26.1.2 this cycle (200 ticks, 2 region-spaced lever circuits, seed 42, period 8): the two driven captures were **frame-for-frame identical for 197/200 frames** — they diverged only in a **3-frame contiguous opening transient** (the cold-CAS warm-up settling), then locked onto the identical trajectory from frame 3 to the end. The harness grades this an honest **CONVERGENT PASS** (exit 0): a transient confined to a small contiguous prefix that then stays locked is deterministic-under-load; ANY divergence *after* convergence would be graded a real FAIL. This proves the DAG is deterministic under a seed-reproducible LIVE toggle stream once the cold-start transient settles — a DISTINCT, weaker claim than the static-world Criterion 1, and it does NOT upgrade Criterion 1 (still static-world). The earlier settle-timing note (a ~13-frame transient on the first manual run) shrank to 3 frames with the harness's 4s inter-run settle; the transient is inherent to a never-settling square wave starting from an empty CAS store, not a determinism bug.
