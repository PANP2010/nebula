...
**Date**: 2026-07-09 (updated — corrected the DG1 Criterion 2 caveat again: the live max of 1 is NOT a "broad dirty set contracting to one layer". `FoliaRegionTickExecutor` dispatches each dirty task individually as `List.of(task)`, so the scheduler is seeded with one task per invocation — the narrowest frontier, never the broad set. The prior "broad-dirty-set / narrow-seeding-would-fix-it" explanation was false by construction; the real cause of live max=1 is left as explicitly unverified this cycle)
**Branch**: feat/fix-folia-scheduler-v2  
**Completion**: ~57% (DG1 now has all three criteria PASSing at multi-region scale: Criterion 1 (10k-tick zero-diff, byte-identical `.nrp` files), Criterion 2 (microsteps ≤256 — `MicroStepRecorder` observed max 1 over 7200 ticks), and Criterion 3 (shadow-overhead budget, p99 1.914ms < 3ms). Two caveats keep DG1 from unconditional acceptance: the Criterion 1 run uses a static captured world (proves capture+hash determinism, not DAG-under-live-load correctness), and the Criterion 2 live max of 1 does NOT exercise deep expansion — but the *reason* is not what earlier cycles wrote. `FoliaRegionTickExecutor` (adapter) dispatches each dirty task to its region thread individually as `List.of(task)` (line 95), so `MicroStepScheduler.executeTick` is seeded with exactly ONE task per invocation — the narrowest possible frontier, never a broad batch. So the live max of 1 is neither "a broad dirty set contracting to one layer" (the scheduler never receives the broad set) nor something harness-level narrow seeding would change (the pipeline already single-task-seeds). Deep expansion (~14 microsteps) IS proven in-process by `MicroStepDepthTest`, which seeds a single leading-edge task into a freshly-unsettled wire and lets it cascade; the reason the same single-task seed does NOT cascade live is unverified this cycle — see the corrected DG1 Criterion 2 note below)
...
- ✅ Microstep-count bound (≤256) at scale — `MicroStepRecorder` + `/nebula perf`, max 1 over 7200 driven DAG ticks (`scripts/perf-harness.sh`, 2026-07-08). Note: the live pipeline feeds the scheduler ONE task per invocation (`FoliaRegionTickExecutor` dispatches `List.of(task)`), so the ≤256 bound is exercised only at its narrowest — deep expansion is not exercised live; see the corrected DG1 Criterion 2 note below
- ✅ Microstep count is governed by dirty-set *shape*, not circuit topology — a single leading-edge seed into a freshly-unsettled straight wire expands over ~14 microsteps, while the same wire seeded whole collapses to ≤1 (`MicroStepDepthTest`, 2026-07-08); see the corrected DG1 Criterion 2 note below
...
- ⚠️ Microstep expansion at scale on a *live* server — the live max of 1 does NOT exercise deep expansion. The cause is subtler than earlier cycles wrote: `FoliaRegionTickExecutor` dispatches each dirty task to its region thread individually as `List.of(task)`, so `MicroStepScheduler.executeTick` is seeded with exactly one task per invocation. The max of 1 is therefore NOT "a broad dirty set contracting in one layer" (the scheduler never receives the broad set), and it is NOT something harness-level narrow seeding would fix (the pipeline already single-task-seeds). Why that single-task seed does not cascade live — most likely because Folia has already propagated the signal before the observe-only shadow runs, so `syncFromNms` reads the already-settled state and no downstream change is detected — is **unverified this cycle** (would need per-invocation task-count + cascade logging on live Folia). Deep expansion (~14 microsteps) is proven in-process by `MicroStepDepthTest` (single leading-edge seed into a freshly-unsettled wire).
...
| Microsteps ≤ 256 per tick | ≤256 | ✅ **VERIFIED AT SCALE** by `scripts/perf-harness.sh`. Large driven run 2026-07-08 (8 region-spaced circuits / 256 components / 7200 DAG ticks): observed **max microsteps = 1** per tick, well under the 256 bound. `/nebula perf` now records the per-tick microstep distribution (`MicroStepRecorder`) and auto-grades PASS/FAIL against `MicroStepScheduler.MAX_MICRO_STEPS`. Note: the max of 1 does NOT come from a broad dirty set contracting to one layer — `FoliaRegionTickExecutor` dispatches each dirty task individually as `List.of(task)`, so `executeTick` is seeded with exactly ONE task per invocation (the narrowest frontier, never a broad batch). The bound and the instrument are confirmed, but deep expansion is not exercised live; why a single-task seed does not cascade live is unverified (see corrected caveat below) |
...
**Verdict**: All three DG1 criteria now have a verified PASS at multi-region scale, though each carries a documented caveat. Criterion 1 (10k-tick zero-diff) **PASSed on real Folia 2026-07-08** — two independent 10k-tick captures are byte-identical (`scripts/zerodiff-harness.sh`, exit 0). Criterion 3 (shadow-overhead budget) PASSed (p99 1.914ms < 3ms). Criterion 2 (microsteps ≤256) **PASSed 2026-07-08** — a large driven workload (8 circuits / 256 components / 7200 DAG ticks) observed a max of 1 microstep per tick, now recorded and auto-graded by `/nebula perf` (`MicroStepRecorder`). **Caveats that keep DG1 from being unconditionally "done":** (a) the Criterion 1 run uses a **static** captured world (see honesty note below) — it proves the capture+hash pipeline is deterministic and exception-free over 10k ticks, not DAG correctness under sustained live redstone; and (b) the Criterion 2 live max of 1 does not exercise deep expansion — but the reason earlier cycles gave for this is **wrong** and is corrected here. `FoliaRegionTickExecutor.executeTasks` dispatches each dirty task to its owning region thread individually (`ownedRunner.run(world, worldName, List.of(task))`, line 95), so `MicroStepScheduler.executeTick` receives exactly one seed task per invocation. The live max of 1 is therefore NOT "a broad dirty set the observe-only pipeline feeds the scheduler, contracting in one SCC-collapsed layer" — the scheduler never receives the broad set — and it is NOT something a torch oscillator / comparator loop OR harness-level narrow seeding would change, because the pipeline **already** single-task-seeds. The most likely real cause (observe-only ordering: Folia propagates the signal before the shadow DAG runs, so `syncFromNms` reads already-settled state and no downstream change is detected) is **unverified this cycle** — confirming it needs per-invocation task-count + cascade logging on live Folia. Deep expansion IS exercised in-process by `MicroStepDepthTest` (single leading-edge seed into a freshly-unsettled wire → ~14 microsteps) and `RedstoneReplayDeterminismTest.singleSourceMicrostepPropagatesWholeLineInOneTick`.
