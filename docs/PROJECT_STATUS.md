...
**Date**: 2026-07-08 (updated — corrected the DG1 Criterion 2 "straight-wire" caveat: microstep depth is governed by dirty-set shape, not circuit topology; verified deterministically by `MicroStepDepthTest`)
**Branch**: feat/fix-folia-scheduler-v2  
**Completion**: ~57% (DG1 now has all three criteria PASSing at multi-region scale: Criterion 1 (10k-tick zero-diff, byte-identical `.nrp` files), Criterion 2 (microsteps ≤256 — `MicroStepRecorder` observed max 1 over 7200 ticks), and Criterion 3 (shadow-overhead budget, p99 1.914ms < 3ms). Two caveats keep DG1 from unconditional acceptance: the Criterion 1 run uses a static captured world (proves capture+hash determinism, not DAG-under-live-load correctness), and the Criterion 2 max of 1 reflects the **broad dirty set** the observe-only pipeline produces — NOT the circuit being "straight wire". `MicroStepDepthTest` proves a straight wire expands over ~14 microsteps under narrow-frontier seeding and collapses to ≤1 when seeded whole, so lifting the live max above 1 needs narrow seeding, not a feedback circuit)
...
- ✅ Microstep-count bound (≤256) at scale — `MicroStepRecorder` + `/nebula perf`, max 1 over 7200 driven DAG ticks (broad-dirty-set workload; `scripts/perf-harness.sh`, 2026-07-08)
- ✅ Microstep count is governed by dirty-set *shape*, not circuit topology — a straight wire narrow-seeded at its leading edge expands over ~14 microsteps, while the same wire seeded whole collapses to ≤1 (`MicroStepDepthTest`, 2026-07-08); see the corrected DG1 Criterion 2 note below
...
- ⚠️ Microstep expansion at scale on a *live* server — the live max of 1 is a property of the broad dirty set the observe-only pipeline produces (Folia touches every wire; the agent records them all), NOT of the circuit topology. Driving the live max above 1 needs **narrow-frontier seeding**, not a different circuit — see the corrected DG1 Criterion 2 note. (`MicroStepDepthTest` proves the ~14-vs-≤1 split deterministically in-process.)
...
| Microsteps ≤ 256 per tick | ≤256 | ✅ **VERIFIED AT SCALE** by `scripts/perf-harness.sh`. Large driven run 2026-07-08 (8 region-spaced circuits / 256 components / 7200 DAG ticks): observed **max microsteps = 1** per tick, well under the 256 bound. `/nebula perf` now records the per-tick microstep distribution (`MicroStepRecorder`) and auto-grades PASS/FAIL against `MicroStepScheduler.MAX_MICRO_STEPS`. Note: the max of 1 is a property of the **broad dirty set** the observe-only pipeline feeds the scheduler (the whole circuit is dirty each tick, so it resolves in one contracted layer) — NOT of "straight wire resolving in one wave". Under narrow-frontier seeding a plain straight wire expands over ~14 microsteps (`MicroStepDepthTest`), so a feedback circuit is not what would lift the live count — narrow seeding is — see corrected caveat below |
...
**Verdict**: All three DG1 criteria now have a verified PASS at multi-region scale, though each carries a documented caveat. Criterion 1 (10k-tick zero-diff) **PASSed on real Folia 2026-07-08** — two independent 10k-tick captures are byte-identical (`scripts/zerodiff-harness.sh`, exit 0). Criterion 3 (shadow-overhead budget) PASSed (p99 1.914ms < 3ms). Criterion 2 (microsteps ≤256) **PASSed 2026-07-08** — a large driven workload (8 circuits / 256 components / 7200 DAG ticks) observed a max of 1 microstep per tick, now recorded and auto-graded by `/nebula perf` (`MicroStepRecorder`). **Caveats that keep DG1 from being unconditionally "done":** (a) the Criterion 1 run uses a **static** captured world (see honesty note below) — it proves the capture+hash pipeline is deterministic and exception-free over 10k ticks, not DAG correctness under sustained live redstone; and (b) the Criterion 2 max of 1 is a property of the **broad dirty set** the observe-only pipeline produces, NOT of straight-wire topology. `MicroStepDepthTest` (2026-07-08) shows the same straight wire expands over ~14 microsteps when only its leading edge is seeded and collapses to ≤1 when the whole line is seeded at once (SCC contraction). So the ≤256 bound and the instrument are confirmed, but the live count of 1 does not exercise deep expansion — and the fix is **narrow-frontier seeding**, not swapping in a torch oscillator / comparator loop (which the broad live dirty set would still collapse). Deep expansion IS exercised in-process by `MicroStepDepthTest` and `RedstoneReplayDeterminismTest.singleSourceMicrostepPropagatesWholeLineInOneTick`.
