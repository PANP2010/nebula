...
import org.nebula.entity.EntityTaskGuardHook;
...
/** Live per-task RW guard for entity actions (B8 C2). */
public final class EntityRwGuardHook implements EntityTaskGuardHook {

    private static final Logger LOG = Logger.getLogger(EntityRwGuardHook.class.getName());
    ...
    public EntityRwGuardHook(RWGuardConfig config) {
        this.config = config;
    }
    ...
    public void afterTask(TaskNode task) {
        ...
        LOG.warning(() -> "RW-GUARD violation: entity task " + task.taskId()
